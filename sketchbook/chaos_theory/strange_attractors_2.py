...
def createAttractor():
    for n in range(NEXAMPLES):
        ...
        for i in range(MAXITERATIONS):
            ...
            x.append(ax[0] + ax[1] * x[i - 1] + ax[2] * x[i - 1] * x[i - 1] + ax[3] * x[i - 1] * y[i - 1] + ax[4] * y[
                i - 1] + ax[5] * y[i - 1] * y[i - 1])
            y.append(ay[0] + ay[1] * x[i - 1] + ay[2] * x[i - 1] * x[i - 1] + ay[3] * x[i - 1] * y[i - 1] + ay[4] * y[
                i - 1] + ay[5] * y[i - 1] * y[i - 1])
        ...
        if drawit:
            if abs(lyapunov) < 10:
                ...
                print
                "periodic {} ".format(lyapunov)
                drawit = False
            else:
                print
                "chaotic {} ".format(lyapunov)
...
def saveAttractor(n, a, b, xmin, xmax, ymin, ymax, x, y):
    ...
    print
    "saved attractor to ./output/{}.png".format(n)
